...
#' @examples
#' fun_hairpin_trimming(Biostrings::DNAString("GGAAAAAAATCTCTCAACA", "+", "AGATCC")
#' @export
fun_hairpin_trimming = function(Hairpin_seq, mut_read_strand, ADAPTOR_SEQ){
  if(mut_read_strand == "+"){
    Hairpin_seq = Biostrings::trimLRPatterns(Rpattern = ADAPTOR_SEQ, subject = Hairpin_seq)
    Hairpin_seq = stringr::str_split(Hairpin_seq, ADAPTOR_SEQ)[[1]]
    if(length(Hairpin_seq)> 1){
      Hairpin_seq = Hairpin_seq[[1]]
    }
    Hairpin_seq = Biostrings::reverseComplement(Biostrings::DNAString(Hairpin_seq))
  }
  else{
    Hairpin_seq = Biostrings::reverseComplement(Hairpin_seq)
    Hairpin_seq = Biostrings::trimLRPatterns(Rpattern = ADAPTOR_SEQ, subject = Hairpin_seq)
    Hairpin_seq = stringr::str_split(Hairpin_seq, ADAPTOR_SEQ)[[1]]
    if(length(Hairpin_seq)> 1){
      Hairpin_seq = Hairpin_seq[[1]]
    }
    Hairpin_seq = Biostrings::DNAString(Hairpin_seq)
  }
  return(Hairpin_seq)
}
